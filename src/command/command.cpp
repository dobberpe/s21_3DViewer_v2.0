...
BgColorCommand::BgColorCommand(Viewer *v_, double r_, double g_, double b_)
    : v(v_),
      ...
      prev_g(v_->bg_g),
      prev_b(v_->bg_b) {
  Logger::instance().log("created bgColor " + QString::number(r) + " " +
...
VertexColorCommand::VertexColorCommand(Viewer *v_, double r_, double g_,
                                       double b_)
    : v(v_),
      r(r_),
      g(g_),
      ...
      prev_g(v_->vertex_g),
      prev_b(v_->vertex_b) {
  Logger::instance().log("created vertexColor " + QString::number(r) + " " +
                         QString::number(g) + " " + QString::number(b));
}
...
PolygonColorCommand::PolygonColorCommand(Viewer *v_, double r_, double g_,
                                         double b_)
    : v(v_),
      r(r_),
      g(g_),
      ...
      prev_g(v_->polygon_g),
      prev_b(v_->polygon_b) {
  Logger::instance().log("created polygonColor " + QString::number(r) + " " +
                         QString::number(g) + " " + QString::number(b));
}
...
VertexSizeCommand::VertexSizeCommand(Viewer *v_, double s)
    : v(v_), size(s), prev_size(v_->vertex_size) {
  Logger::instance().log("created vertexSize " + QString::number(size));
}

VertexSizeCommand::VertexSizeCommand(const VertexSizeCommand &first,
  ...
  if (first.v == last.v) {
    ...
    throw exception();
  Logger::instance().log("created combined vertexSize " +
                         QString::number(size));
}
...
LineWidthCommand::LineWidthCommand(Viewer *v_, double s)
    : v(v_), size(s), prev_size(v_->line_width) {
  Logger::instance().log("created lineWidth " + QString::number(size));
}

LineWidthCommand::LineWidthCommand(const LineWidthCommand &first,
                                   const LineWidthCommand &last) {
  if (first.v == last.v) {
    v = first.v;
    prev_size = first.prev_size;
    size = last.size;
...
ProjectionTypeCommand::ProjectionTypeCommand(Viewer *v_, int t)
    : v(v_), type(t), prev_type(v_->projection_type) {
...
VertexTypeCommand::VertexTypeCommand(Viewer *v_, int t)
    : v(v_), type(t), prev_type(v_->vertex_type) {
...
LineTypeCommand::LineTypeCommand(Viewer *v_, int t)
    : v(v_), type(t), prev_type(v_->line_type) {
...
void CommandManager::combineCommand(ICommand *command) {
  ...
  if (combine_stopper || history.empty())
    ...
    if (dynamic_cast<RotateCommand *>(command) &&
      ...
      combined_command =
          new RotateCommand(*(dynamic_cast<RotateCommand *>(prev_command)),
                            *(dynamic_cast<RotateCommand *>(command)));
    else if (dynamic_cast<MoveCommand *>(command) &&
             dynamic_cast<MoveCommand *>(prev_command))
      combined_command =
          new MoveCommand(*(dynamic_cast<MoveCommand *>(prev_command)),
                          *(dynamic_cast<MoveCommand *>(command)));
    else if (dynamic_cast<ScaleCommand *>(command) &&
             dynamic_cast<ScaleCommand *>(prev_command))
      combined_command =
          new ScaleCommand(*(dynamic_cast<ScaleCommand *>(prev_command)),
                           *(dynamic_cast<ScaleCommand *>(command)));
    else if (dynamic_cast<VertexSizeCommand *>(command) &&
             dynamic_cast<VertexSizeCommand *>(prev_command))
      combined_command = new VertexSizeCommand(
          *(dynamic_cast<VertexSizeCommand *>(prev_command)),
          *(dynamic_cast<VertexSizeCommand *>(command)));
    else if (dynamic_cast<LineWidthCommand *>(command) &&
             dynamic_cast<LineWidthCommand *>(prev_command))
      combined_command = new LineWidthCommand(
          *(dynamic_cast<LineWidthCommand *>(prev_command)),
          *(dynamic_cast<LineWidthCommand *>(command)));
    else
      executeCommand(command);
