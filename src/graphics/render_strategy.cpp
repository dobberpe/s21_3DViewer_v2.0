...
void VertexRenderStrategy::render(Viewer *viewer) {
  if (viewer->vertex_type != NONE) {
    ...
    auto [vertex_array, amount_vertex] =
        viewer->get_worker()->get_vertex_array();
    glVertexPointer(3, GL_DOUBLE, 0, vertex_array);
    glDrawArrays(GL_POINTS, 0, amount_vertex);
    if (viewer->vertex_type == ROUND) glDisable(GL_POINT_SMOOTH);
  }
}
...
void PolygonRenderStrategy::render(Viewer *viewer) {
  ...
  auto [polygon_array, polygon_amount] =
      viewer->get_worker()->get_polygon_array();
  ...
  if (viewer->line_type == DASH_LINE) glDisable(GL_LINE_STIPPLE);
}
