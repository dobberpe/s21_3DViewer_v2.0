...
void Viewer::mouseMoveEvent(QMouseEvent *event) {
  ...
  if (event->buttons() & Qt::LeftButton) {
    ...
    CommandManager::instance().executeCommand(
        new RotateCommand(this, new_pos.y() * 0.00001 * move_coef,
                          new_pos.x() * 0.00001 * move_coef, 0, false));
  } else if (event->buttons() & Qt::RightButton) {
    Logger::instance().log("mouse move");
    CommandManager::instance().combineCommand(
        new MoveCommand(this, new_pos.x() * 0.00001 * move_coef,
                        -new_pos.y() * 0.00001 * move_coef, 0, false));
  }
}
